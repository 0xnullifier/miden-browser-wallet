...
export const createBalanceStore = () => create<BalanceState, [["zustand/immer", never]]>((set, get) => ({
    ...
    loadBalance: async (client, _accountId) => {
        ...
        await Promise.all(accountRecord.vault().fungibleAssets().map(async (asset) => {
            let tokenInfo = faucets.find((faucet) => faucet.address === asset.faucetId().toString())
            if (!tokenInfo) {
            ...
            const balance = Number(balanceInBaseDenom) / 10 ** tokenInfo.decimals;
    ...
    faucet: async (accountId, amount) => {
        ...
        try {
            const amountInBaseDenom = BigInt(Math.trunc(Number(amount) * (10 ** DECIMALS)))
