...
import { Search, Activity, Clock, Users, FileText, Droplets, ArrowUpRight, ArrowDownLeft } from 'lucide-react'
        ...
        <div className='w-full px-4 md:px-8 max-w-6xl mx-auto'>
            <div className="bg-background">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4">
                    <StatsCard icon={Activity} title="Total Transactions" value={stats.totalTransactions} color="blue" />
                    <StatsCard icon={Clock} title="Transactions (1h)" value={stats.transactionsLastHour} color="green" />
                    <StatsCard icon={Users} title="Accounts Created" value={stats.accountsCreated} color="purple" className="hidden md:block" />
                    <StatsCard icon={FileText} title="Notes Created" value={stats.notesCreated} color="orange" className="hidden md:block" />
                    <StatsCard icon={Droplets} title="Faucet Requests" value={stats.faucetRequests} color="cyan" className="hidden md:block" />
                </div>
            </div>
...
function StatsCard({ icon: Icon, title, value, color, className = "" }: { icon: React.ElementType; title: string; value: string; color: string; className?: string }) {
    ...
    return (
        <div className={`p-3 rounded-lg border border-border/40 bg-card/30 hover:bg-card/50 transition-colors ${className}`}>
...
function TransactionList({ transactions, isMobile = false }: { transactions: BackendTransaction[]; isMobile?: boolean }) {
