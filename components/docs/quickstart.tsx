...
export function QuickStart() {
    return (
        <div className="w-full px-4 sm:px-6 md:px-8 py-4 mx-auto max-w-none lg:max-w-4xl">
            ...
            <p className="text-lg sm:text-xl md:text-2xl font-bold pt-8 pb-2 text-wrap hyphens-auto" id="accounts">Accounts</p>
            <div className="text-foreground text-sm sm:text-base text-wrap leading-relaxed hyphens-auto"> Accounts on Miden are a complex entity but for user-facing apps they are nothing more than a simple address. Like Ethereum EOAs, they are capable of holding assets but can also store data and execute custom code. </div>
            <p className="pt-2 text-sm sm:text-base text-wrap leading-relaxed hyphens-auto">The web SDK provides a simple interface to manage these accounts, creating accounts and fetching accounts:</p>
            <div className="py-4">
                <CodeBlock
                    language="typescript"
                    filename="account.ts"
            ...
            <p className="text-lg sm:text-xl md:text-2xl font-bold pt-4 pb-2 text-wrap hyphens-auto" id="tokens">Tokens</p>
            ...
            <p className="text-base sm:text-lg md:text-xl font-bold pt-4 pb-2 text-wrap hyphens-auto" id="tokens-minting">Minting Tokens</p>
            <p className="text-foreground text-sm sm:text-base text-wrap leading-relaxed hyphens-auto"> The minting of tokens is handled by a special type of account called a <strong>faucet account</strong>. These faucets create notes that can be consumed by the receiver. The faucet ID or the account ID for the faucet account can be thought of as the token address for the asset. The code for creating faucet accounts and fetching faucet accounts via the SDK: </p>
            <div className="py-4">
                <CodeBlock
                    language="typescript"
                    filename="mint.ts"
            ...
            <p className="text-base sm:text-lg md:text-xl font-bold pt-4 pb-2 text-wrap hyphens-auto" id="consuming">Consuming Notes</p>
            <p className="text-foreground text-sm sm:text-base text-wrap leading-relaxed hyphens-auto"> The consuming of public notes is pretty straightforward, but for private notes you would have to get the serialized {<INLINE_CODE text="NoteFile" />} via the {<INLINE_CODE text="client.exportNote(noteId)" />} method and send those bytes via some communication channel (for example our app uses WebRTC) and then import them via {<INLINE_CODE text="client.importNote(noteBytes)" />}. The code below consumes <strong>all the notes</strong> found by the client, both public and private: </p>
            <div className="py-4">
                <CodeBlock
                    language="typescript"
                    filename="consume.ts"
            ...
            <p className="text-base sm:text-lg md:text-xl font-bold pt-4 pb-2 text-wrap hyphens-auto" id="send">Sending Tokens</p>
            <p className="text-foreground text-sm sm:text-base text-wrap leading-relaxed hyphens-auto"> Sending tokens is pretty simple in the SDK: </p>
            <div className="py-4">
                <CodeBlock
                    language="typescript"
                    filename="send.ts"
