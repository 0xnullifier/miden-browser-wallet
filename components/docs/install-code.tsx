...
export function InstallComponent({ packageManagers }: { packageManagers: PackageManager[] }) {
            ...
            <Tabs defaultValue={activeTab} value={activeTab} onValueChange={setActiveTab} className="w-full">
                <div className="flex items-center justify-between border-b border-border bg-muted/10">
                    ...
                    <div className="p-2">
                        <Button
                            ...
                            onClick={() => {
                                const activeCommand = packageManagers.find((pm) => pm.id === activeTab)?.command || packageManagers[0].command
                                copyToClipboard(activeCommand, "install-command")
                            }}
                        >
                            {copiedStates["install-command"] ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                        </Button>
                    </div>
                </div>
                ...
                {packageManagers.map((pm) => (
                    <TabsContent key={pm.id} value={pm.id} className="m-0">
                        <div className="p-4 font-mono text-foreground bg-muted/5">{pm.command}</div>
                    </TabsContent>
                ))}
            </Tabs>
        </div>
