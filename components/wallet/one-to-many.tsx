...
export function OneToMany({ isOneToMany, setIsOneToMany, amount, receipient, selectedFaucetId }: OneToManyProps) {
    ...
    const setSelectedFaucetForRow = (id: string) => {
        return (faucetId: FaucetInfo) => setRows(rows.map((row) => (row.id === id ? { ...row, faucetId } : row)))
    }
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true)
        try {
            const txResult = await sendToMany(account, rows.map(row => ({ to: row.address, amount: BigInt(Math.trunc(parseFloat(row.amount))), faucet: row.faucetId })))
                    ...
                    <div className="space-y-3">
                        {rows.map((row, index) => (
                            <div key={row.id} className="grid grid-cols-12 gap-2 items-center">
                                ...
                                <div className="col-span-2">
                                    <TickerDropdown selectedTicker={row.faucetId} setSelectedTicker={setSelectedFaucetForRow(row.id)} />
                                </div>
